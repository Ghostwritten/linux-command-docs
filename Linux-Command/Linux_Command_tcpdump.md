...
可以给定的值包括`src/dst/src or dst/src and dst`，默认为src or dst。例如，"src foo"表示源主机为foo的数据包，"dst net 128.3"表示目标网络为128.3的数据包，"src or dst port 22"表示源或目的端口为22的数据包。
...
除了使用修饰符和ID组成的表达式单元，还有关键字表达式单元：`gateway`，`broadcast`，`less`，`greater`以及算术表达式。
...
### 4.4 打印helios<-->hot或helios<-->ace之间通信的数据包

```bash
tcpdump host helios and \( hot or ace \)
```

### 4.5 打印ace与任何其他主机之间通信的IP数据包
但不包括与helios之间的数据包
...
### 4.6 截获主机hostname发送的所有数据

```bash
tcpdump src host hostname
```

### 4.7 监视所有发送到主机hostname的数据包
...
### 4.9. 对本机的udp 123端口进行监视(123为ntp的服务端口)		
